...
function sign(method: string, url: URL, body: string, key: string, secret: string) {
  ...
  const signature = hmac(hmac(hmac(hmac(`AWS4${secret}`, stamp), 'auto'), 's3'), 'aws4_request')
  return {
    Authorization: `AWS4-HMAC-SHA256 Credential=${key}/${scope}, SignedHeaders=${signed}, Signature=${hmac(signature, hash(canonical)).toString('hex')}`,
